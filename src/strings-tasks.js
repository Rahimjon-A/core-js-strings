...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < newArr.length; i += 1) {
    if (newArr[i].length > count.length) {
      count = newArr[i];
    }
  }
  return count;
...
function extractNameFromTemplate(value) {
  ...
  return newVal.slice(0, newVal.length - 1);
}
...
function encodeToRot13(str) {
  const alfabit = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  let rot13 = '';

  for (let i = 0; i < str.length; i += 1) {
    const char = str[i];
    const isUpperCase = char === char.toUpperCase();
    const index = alfabit.indexOf(char.toUpperCase());

    if (index !== -1) {
      const newIndex = (index + 13) % 26;
      const newChar = alfabit[newIndex];
      rot13 += isUpperCase ? newChar : newChar.toLowerCase();
    } else {
      rot13 += char;
    }
  }
